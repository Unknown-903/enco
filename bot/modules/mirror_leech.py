...
class Mirror(TaskListener):
    def __init__(self, client: Client, message: Message, isQbit=False, isLeech=False, vidMode=None, sameDir=None, bulk=None, multiTag=None, options=''):
        if sameDir is None:
            sameDir = {}
        if bulk is None:
            bulk = []
    ...
    async def newEvent(self):
        ...
        if isinstance(reply_to, list):
            ...
            Mirror(self.client, nextmsg, self.isQbit, self.isLeech, self.vidMode, self.sameDir, self.bulk, self.multiTag, self.options).newEvent()
        ...
        if (not self.isQbit and not is_magnet(self.link) and not is_rclone_path(self.link) and
...
# Only /leech command is registered — supports -vt flag for video tools
