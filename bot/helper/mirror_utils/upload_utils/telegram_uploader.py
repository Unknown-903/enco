...
class TgUploader:
    ...
    async def _upload_file(self, caption, file, force_document=False):
        if self._send_msg is None:
            LOGGER.error('_upload_file: _send_msg is None, cannot upload. Path: %s', self._up_path)
            raise AttributeError('_send_msg is None - no valid chat to upload to')
    ...
    async def _msg_to_reply(self):
        if self._leech_log and self._leech_log != self._listener.message.chat.id:
            ...
            try:
                ...
                LOGGER.warning('_msg_to_reply get_messages failed: %s, falling back to listener.message', e)
                self._send_msg = None
            if not self._send_msg or not self._send_msg.chat:
                self._send_msg = self._listener.message
        # upDest copy happens per-file in _upload_file, not here
