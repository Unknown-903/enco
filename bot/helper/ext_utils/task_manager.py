...
async def check_limits_size(listener, size, playlist=False, play_count=False):
    ...
    # Ensure numeric types - config values may come as strings from DB
    try: megadl  = float(megadl)  if megadl  else 0
    except: megadl  = 0
    try: torddl  = float(torddl)  if torddl  else 0
    except: torddl  = 0
    try: zuzdl   = float(zuzdl)   if zuzdl   else 0
    except: zuzdl   = 0
    try: leechdl = float(leechdl) if leechdl else 0
    except: leechdl = 0
    try: storage = float(storage) if storage else 0
    except: storage = 0
    try: max_pyt = int(max_pyt)   if max_pyt else 0
    except: max_pyt = 0
    if config_dict['PREMIUM_MODE'] and not is_premium_user(listener.user_id):
        mdl = torddl = zuzdl = leechdl = float(config_dict['NONPREMIUM_LIMIT'] or 0)
        megadl = min(float(megadl or 0), float(mdl or 0)) or None
        max_pyt = 10
